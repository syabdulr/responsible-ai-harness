...
function cleanOutcome(caseId: string): CaseOutcome {
  return {
    ...
    judgeResult: {
      ...
      modelMetadata: { jev_model: "jev-latest", jev_usage_input_tokens: 10, jev_usage_output_tokens: 2, jev_latency_ms: 55, "jev_q_prompt_injection_instruction_hierarchy_v1_probability": 0.05 },
    },
    judgeError: undefined,
    finding: undefined,
    review: undefined,
...
describe("report.json contract", () => {
  ...
  it("carries per-question probabilities for a Jev-judged case", () => {
    ...
    expect(caseB?.judge?.perQuestionProbabilities).toEqual([{ questionId: "prompt_injection_instruction_hierarchy_v1", probability: 0.05 }]);
